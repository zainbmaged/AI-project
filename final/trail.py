...
def evaluate2(board,player):
    row_matrix_sum=[0]*4
    col_matrix_sum=[0]*4
    diagonal_matrix_sum=[0]*2
    player=int(player)*-2+1

    for row in range(1,5):
        for col in range(1,5):
            is_player=int((board[row][col][-1]%2==(player+1)/2) and (board[row][col][-1]!=0))
            row_matrix_sum[row-1]+=is_player
            col_matrix_sum[col-1]+=is_player
            if(row==col):
                diagonal_matrix_sum[0]+=is_player
            elif(row+col==5):
                diagonal_matrix_sum[1]+=is_player

    if(max(max(row_matrix_sum),max(col_matrix_sum),max(diagonal_matrix_sum))==4):
        return -player
    return 0


def evaluate3(board,player):
    # sum for the black player (minmizer)
    row_matrix_sum_black=[0]*4
    col_matrix_sum_black=[0]*4
    diagonal_matrix_sum_black=[0]*2

    # sum for the white player (maximizer)
    ...
    whight_max=max(max(row_matrix_sum_white),max(col_matrix_sum_white),max(diagonal_matrix_sum_white))*250*-player
    black_max=max(max(row_matrix_sum_black),max(col_matrix_sum_black),max(diagonal_matrix_sum_black))*250*player
    return max(whight_max,black_max,key=abs)*-player
